...
const InboxMessage = ({ name, message, image }: Props) => {
  return (
    <div className="cursor-pointer transition-all duration-300 hover:bg-gray-100">
      <div className="flex gap-2 border-b py-2 px-4">
        <Avatar src={image} alt="Avatar" size="medium" />

        <div className="flex flex-col ">
          <h4 className="max-h-[16px] w-60 overflow-hidden text-sm font-semibold">
            {name}
          </h4>
          <p className="mt-1 max-h-[40px] w-60 overflow-hidden text-sm">
            {message}
          </p>
